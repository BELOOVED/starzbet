...
export const Platform_PlayerBonuses_Query: TDocument<TPlatform_PlayerBonuses_QueryOptionalFields> = process.env.GRAPHQL_PERSISTED === "strict"
        ? () => () => ""
        : createDocument<TPlatform_PlayerBonuses_QueryOptionalFields>("query Platform_PlayerBonuses($where: Platform_PlayerBonusWhereInput, $cursor: Cursor, $orderBy: [Platform_PlayerBonusOrderBy!]) { platform @normalize { PlayerBonuses(where: $where, cursor: $cursor, orderBy: $orderBy) { ...Platform_PlayerBonusesPage } } }", {
...
Platform_PlayerBonuses_Query.extra = createDocument<TPlatform_PlayerBonuses_QueryOptionalFields>(
  process.env.GRAPHQL_PERSISTED === "strict" ? "" : "query Platform_PlayerBonuses($where: Platform_PlayerBonusWhereInput, $cursor: Cursor, $orderBy: [Platform_PlayerBonusOrderBy!]) { platform @normalize { PlayerBonuses(where: $where, cursor: $cursor, orderBy: $orderBy) { __typename pageInfo { __typename total } } } }",
  {},
);
